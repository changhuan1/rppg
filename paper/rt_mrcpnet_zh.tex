...
人脸视频为这个矛盾提供了一个可能的入口。除了表情、视线和头部运动，人脸视频还包含由血容量变化引起的细微皮肤颜色波动。远程光电容积脉搏波研究已经表明，这些波动可以用于心率估计、心率变异性分析、情绪状态监测和压力相关推断~\cite{mcduff2014remote,benezeth2018remotehrv,nikolaiev2020noncontact}。近年来，UBFC-Phys、StressID 和 ForDigitStress 等数据集进一步将人脸视频放入社会压力或数字交互协议中，并同步采集接触式生理信号、主观评分和任务标签~\cite{sabour2021ubfcphys,chaptoukaev2023stressid,heimerl2024fordigitstress}。这说明普通摄像头中可能蕴含足以支持压力识别的生理证据；困难在于，这些证据非常弱、非常局部，也非常容易被无关变化混淆。
...
本文围绕这个观察展开：将人脸看成一个平均信号，可能会把有用的局部颜色动态和不可靠区域混在一起；将整个片段做统一时间聚合，也可能把短时有效反应与眨眼、转头或光照突变混在一起。因此，本文将不同面部区域视为多路弱生理观测源。每个视频片段被表示为多条 RGB 颜色时序序列，模型只需要回答两个具体问题：每个区域中哪些时间位置更可靠，当前片段中哪些区域更值得信任。这个设计位于沉重的完整视频建模和刚性的人工生理特征流程之间，更适合小规模社会压力数据上的 subject-independent 评估。
...
早期非接触式压力研究通常将远程生理信号作为中间表示。McDuff 等人~\cite{mcduff2014remote} 证明，可以从人脸视频中估计心率、呼吸和心率变异性等生理参数，并用于认知压力分类。Nikolaiev 等人~\cite{nikolaiev2020noncontact} 进一步研究了基于普通摄像头视频的远程光电容积脉搏波压力检测流程，并从视频中恢复心率变异性指标。Benezeth 等人~\cite{benezeth2018remotehrv} 探索了远程心率变异性在情绪状态监测中的应用。这些工作给出了关键证据：普通摄像头并非只能记录外观，也可以捕获与压力相关的弱生理变化。

近年来，相关工作开始从“先估计生理指标再分类”转向直接视频学习。Xu 等人~\cite{xu2024mtasr} 提出基于多任务学习和峰值注意力的非接触式压力识别方法，说明面向压力任务的注意力机制已经成为一个重要方向。SympCam~\cite{braun2024sympcam} 及其相关研究~\cite{braun2023sympathetic} 则利用面部和手部视频远程估计交感唤醒，并用于身体压力检测。这些研究表明，压力识别不是简单的心率估计问题。判别证据可能表现为局部颜色动态、短时峰值、外周血流变化，也可能混入任务诱发的说话和运动模式中。因此，模型不应只追求一条“最干净”的全局脉搏波，而应保留区域级时间证据，并学习如何在当前片段中使用这些证据。

\subsection{人脸视频远程生理测量}

远程生理测量为非接触式压力识别提供了技术基础。传统远程光电容积脉搏波算法通常通过颜色空间投影、盲源分离或局部一致性建模抑制光照和运动干扰，代表性方法包括 CHROM~\cite{dehaan2013chrom}、POS~\cite{wang2017pos} 和 LGI~\cite{pilz2018lgi}。这些方法效率高、解释性强，但性能依赖皮肤区域分割、光照假设和后处理策略。

深度学习方法则尝试从视频中自动学习时空表示。DeepPhys~\cite{chen2018deepphys} 引入卷积注意力机制进行视频生理测量；PhysNet~\cite{yu2019physnet} 使用时空网络恢复远程脉搏信号；MTTS-CAN~\cite{liu2020mttscan} 利用多任务时序移位注意力同时估计心血管和呼吸相关信号；EfficientPhys~\cite{liu2023efficientphys} 进一步强调简单、快速的摄像头心脏测量。近年来，Transformer 和序列模型也被广泛引入。PhysFormer 通过时间差分 Transformer 建模远程生理信号~\cite{yu2022physformer}；RhythmFormer 利用周期稀疏注意力挖掘远程脉搏信号的周期性~\cite{zou2025rhythmformer}；Masked Attention Regularization 面向运动鲁棒性改进远程生理测量~\cite{zhao2024mar}；PhysMamba、MultiPhys 和 DualPhysMamba 等方法进一步探索状态空间模型和混合序列结构在长序列生理建模中的效率~\cite{luo2024physmamba,huo2025multiphys,li2026dualmamba}；FactorizePhys 则研究远程生理感知中的多维注意力建模~\cite{joshi2024factorizephys}。
...
每个区域序列由共享的一维时序编码器进行编码。对于一个批量输入，首先将多区域张量展开为 $(B K) \times C \times T$，然后输入由多层一维卷积构成的时序编码器：
\begin{equation}
    H = E_{\theta}(X),
\end{equation}
经过维度调整后得到 $H \in \mathbb{R}^{B K \times T \times D}$，其中 $D$ 为隐藏维度。所有区域共享同一个编码器有两个原因。第一，不同面部区域的低层颜色时序变化具有相似结构。第二，共享参数可以降低模型复杂度，减少小规模压力数据上的过拟合风险。
...
\begin{itemize}
    ...
    \item 传统 rPPG 特征分类器：从面部颜色轨迹中提取 POS 或 CHROM 风格信号，再计算 HR/HRV 等手工特征，并使用 SVM 或随机森林分类。
    \item 轻量视频时序基线：使用紧凑 CNN-LSTM 或 3D CNN 直接输入裁剪后的人脸片段，输出压力类别。
    \item 已有 rPPG backbone 加压力分类头：将 PhysNet 或 EfficientPhys 等代表性 rPPG 模型的输出头替换为压力分类头。
...
表~\ref{tab:main_results_zh} 和图~\ref{fig:quantitative_visualization_zh} 表明，RT-MRCPNet 在 RGB 序列、传统 rPPG 特征和轻量视频基线中取得最佳结果。左右脸颊单区域模型优于鼻部和下巴区域，说明不同面部区域的信号质量并不均匀。RT-MRCPNet 进一步优于多区域固定融合，说明其收益不仅来自使用更多 ROI，也来自时间位置和面部区域的自适应加权。

\subsection{消融实验}

消融实验进一步拆解本文模型，并采用与表~\ref{tab:main_results_zh} 相同的协议。与主对比实验不同，主对比实验改变的是输入表示或融合策略；消融实验则从 RT-MRCPNet 内部移除具体注意力模块，用于检验每个组件是否对最终分类有贡献。
...
表~\ref{tab:ablation_zh} 表明两个注意力阶段具有互补作用。移除两个注意力模块后，模型退化为固定多区域融合；单独加入时间注意力或区域注意力均能提升性能，同时使用二者时取得最佳结果。
...
RT-MRCPNet 使用紧凑的区域 RGB 序列，而不是完整人脸视频帧。其输入规模为 $K \times T \times 3$，远小于完整视频模型的 $T \times H \times W \times 3$。表~\ref{tab:complexity_zh} 报告了代表性方法的参数量、FLOPs 和推理时间。
