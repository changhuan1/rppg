...
\end{equation}
This binary setting is clear and reproducible, and it matches the central focus of this paper on whether a video model can distinguish resting clips from social-stress task clips without contact sensors.
...
The comparison experiments follow a unified protocol. Whenever the input form permits, all compared methods use the same input source, the same subject-independent split, the same clip construction, and the same evaluation code. These controlled comparisons evaluate whether multi-region temporal representation with attention fusion provides substantial improvements when all inputs are restricted to facial RGB video. The baselines range from simple color-sequence models to video temporal models and rPPG-related models.
